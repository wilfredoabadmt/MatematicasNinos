  ...
  return (
    <div className="min-h-screen bg-[#FFF9EC] relative overflow-hidden flex flex-col items-center justify-between p-3 md:p-6 text-[#241A3D]">
      ...
      <div className="relative z-10 w-full max-w-4xl flex flex-col items-center my-auto">
        ...
        <div className="w-full flex flex-wrap items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-white hover:bg-[#FFF3D9] text-[#35206F] rounded-full px-4 py-2 text-sm font-extrabold transition-all shadow-xs border-2 border-[#FFC928]/50 font-nunito"
            >
              <span>🔄</span> <span>Cambiar Compañero</span>
            </button>
            {onOpenRegistration && (
              <button
                onClick={onOpenRegistration}
                className="flex items-center gap-2 bg-white hover:bg-[#FFF3D9] text-[#35206F] rounded-full px-4 py-2 text-sm font-extrabold transition-all shadow-xs border-2 border-[#7AC943]/50 font-nunito"
              >
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 bg-[#FFC928] text-[#35206F] rounded-full px-4 py-1.5 text-sm sm:text-base font-extrabold shadow-xs font-nunito border border-white">
              <span className="text-base">🥚</span>
              <span>{totalEggs} Misiones</span>
            </div>
            <div className="flex items-center gap-1.5 bg-[#7AC943] text-white rounded-full px-4 py-1.5 text-sm sm:text-base font-extrabold shadow-xs font-nunito border border-white">
              <span className="text-base">⭐</span>
        ...
        <div className="w-full bg-white rounded-3xl p-4 sm:p-5 mb-3 sm:mb-4 shadow-kg border-2 border-[#FFC928]/50 flex items-center gap-4 animate-[fadeIn_0.5s_ease-out]">
          <div
            className="flex-shrink-0 cursor-pointer"
            onClick={() => speakAndWait(hero.greeting)}
          >
            <HeroAvatar heroId={hero.id} size={75} talking={heroTalking} hasCrown={hasCrown} />
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-base sm:text-lg font-bold text-[#35206F] font-fredoka">
                {hero.emoji} {hero.name} {hasCrown && '👑'}
              </span>
              <span className="text-xs bg-[#7AC943]/20 text-[#2F6B12] font-extrabold px-2.5 py-0.5 rounded-full font-nunito">
                Compañero Activo
              </span>
            </div>
        ...
        <div className="text-center mb-3">
          <h1 className="text-3xl sm:text-5xl font-bold font-fredoka text-[#35206F] mb-1">
            KidGenius <span className="text-[#7AC943]">Club</span> 🚀
          </h1>
          <p className="text-sm sm:text-base text-[#554A6D] font-bold font-nunito">
        ...
        {showDifficulty && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-center mb-1 text-[#35206F] font-fredoka">
                ¿Qué nivel quieres practicar hoy? 🤔
              </h2>
              <p className="text-center text-[#554A6D] mb-4 text-sm font-bold font-nunito">
              ...
              <div className="space-y-3">
                {difficulties.map(diff => (
                  ...
                  >
                    <div>{diff.label}</div>
                    <span className="block text-xs sm:text-sm font-bold opacity-95 font-nunito">{diff.sub}</span>
                  </button>
                ))}
              </div>
              <button
                onClick={() => setShowDifficulty(false)}
                className="mt-4 w-full text-[#554A6D] hover:text-[#35206F] py-2 text-sm font-extrabold font-nunito text-center"
              >
                ← Volver a misiones
              </button>
            </div>
        ...
        <div className="w-full max-w-4xl mb-3 sm:mb-4 bg-white rounded-3xl p-4 sm:p-5 border-2 border-[#FFC928]/50 shadow-kg animate-[fadeIn_0.5s_ease-out]">
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="flex items-center gap-2">
              <span className="text-2xl">🗺️</span>
              <h2 className="text-sm sm:text-base font-bold text-[#35206F] font-fredoka">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3">
            <div className="bg-[#FFF9EC] rounded-2xl p-3 text-center border-2 border-[#FFC928]/40 shadow-2xs">
              <span className="text-xl sm:text-2xl block mb-1">🎨📄</span>
              <span className="text-xs sm:text-sm font-extrabold text-[#241A3D] block leading-snug font-nunito">Lámina PDF</span>
              <span className="text-xs text-[#554A6D] font-bold block font-nunito mt-0.5">5 Misiones</span>
            </div>

            <div className="bg-[#FFF9EC] rounded-2xl p-3 text-center border-2 border-[#FFC928]/40 shadow-2xs">
              <span className="text-xl sm:text-2xl block mb-1">{isPuzzleUnlocked ? '🔓🧩' : '🔒🧩'}</span>
              <span className="text-xs sm:text-sm font-extrabold text-[#241A3D] block leading-snug font-nunito">Rompecabezas</span>
              <span className="text-xs text-[#554A6D] font-bold block font-nunito mt-0.5">{isPuzzleUnlocked ? '¡LIBERADO!' : `${totalEggs}/12 Misiones`}</span>
            </div>

            <div className="bg-[#FFF9EC] rounded-2xl p-3 text-center border-2 border-[#FFC928]/40 shadow-2xs">
              <span className="text-xl sm:text-2xl block mb-1">{isDiplomaUnlocked ? '📜🏆' : '🔒📜'}</span>
              <span className="text-xs sm:text-sm font-extrabold text-[#241A3D] block leading-snug font-nunito">Diploma Oficial</span>
              <span className="text-xs text-[#554A6D] font-bold block font-nunito mt-0.5">{isDiplomaUnlocked ? '¡LIBERADO!' : `${totalEggs}/15 Misiones`}</span>
            </div>

            <div className="bg-[#FFF9EC] rounded-2xl p-3 text-center border-2 border-[#FFC928]/40 shadow-2xs">
              <span className="text-xl sm:text-2xl block mb-1">{hasCrown ? '👑✨' : '🔒👑'}</span>
              <span className="text-xs sm:text-sm font-extrabold text-[#241A3D] block leading-snug font-nunito">Corona de Genio</span>
              <span className="text-xs text-[#554A6D] font-bold block font-nunito mt-0.5">{hasCrown ? '¡PUESTA!' : `${totalEggs}/20 Misiones`}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {isPuzzleUnlocked && (
              <button
                ...
                className="w-full bg-gradient-to-r from-[#35206F] via-[#4B2C99] to-[#7AC943] text-white rounded-2xl py-3 px-4 text-sm font-extrabold transform hover:scale-102 active:scale-98 transition-all shadow-md font-nunito flex items-center justify-center gap-2"
              >
            ...
            {isDiplomaUnlocked && (
              <button
                ...
                className="w-full bg-gradient-to-r from-[#FFC928] via-[#FF8A25] to-[#7AC943] text-white rounded-2xl py-3 px-4 text-sm font-extrabold transform hover:scale-102 active:scale-98 transition-all shadow-md font-nunito flex items-center justify-center gap-2"
              >
              ...
              return (
                <button
                  ...
                  className={`bg-white rounded-3xl p-5 md:p-6
                    ...
                    group relative overflow-hidden border-2 ${isUnlocked ? 'border-[#7AC943] ring-2 ring-[#7AC943]/40' : 'border-[#FFC928]/40 hover:border-[#FFC928]'}`}
                  style={{ animation: `fadeInUp 0.4s ease-out ${index * 0.06}s both` }}
                >
                  {isUnlocked && (
                    <div className="absolute top-2 right-2 bg-[#7AC943] text-white font-extrabold text-xs px-2.5 py-0.5 rounded-full shadow-xs font-nunito animate-pulse">
                      🔓 RETO DOMINADO
                    </div>
                  )}

                  <div className="relative text-center">
                    <div className="text-4xl md:text-5xl mb-2 transform group-hover:scale-115 transition-transform duration-300">{cat.emoji}</div>
                    <h3 className="text-base md:text-lg font-bold text-[#35206F] mb-1 font-fredoka leading-snug">{cat.title}</h3>
                    <p className="text-xs sm:text-sm text-[#554A6D] font-bold mb-3 font-nunito leading-relaxed">{cat.description}</p>

                    <div className="inline-flex items-center justify-center gap-1.5 bg-[#FFF9EC] rounded-full px-4 py-1 border border-[#FFC928]/60 shadow-2xs">
                      <span className="text-sm">🥚</span>
                      <span className="text-xs sm:text-sm font-extrabold text-[#35206F] font-nunito">
