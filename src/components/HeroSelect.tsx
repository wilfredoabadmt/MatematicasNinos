...
const HeroSelect: React.FC<HeroSelectProps> = ({ onSelectHero, activeProfile }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FFF9EC] relative overflow-hidden flex flex-col justify-between p-4 sm:p-6 text-[#241A3D]">
      {/* Background Shapes */}
      ...
      <div className="relative z-10 max-w-4xl mx-auto w-full my-auto flex flex-col items-center">
        <div className="text-center mb-6 animate-[fadeInDown_0.6s_ease-out]">
          <div className="inline-flex items-center gap-1.5 bg-[#35206F] text-white px-4 py-1.5 rounded-full text-xs sm:text-sm font-extrabold shadow-sm mb-2 font-nunito">
            <span>✨</span>
            <span>ELIGE TU COMPAÑERO DE AVENTURA</span>
            <span>✨</span>
          </div>

          <h1 className="text-3xl sm:text-5xl font-bold text-[#35206F] font-fredoka mb-1">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3.5 sm:gap-6 w-full">
          {heroes.map((hero, index) => {
            ...
            return (
              ...
              >
                {isFeatured && (
                  <div className="absolute top-2.5 right-2.5 bg-[#7AC943] text-white text-xs font-extrabold px-2.5 py-0.5 rounded-full font-nunito shadow-xs animate-pulse">
                    ★ MASCOTA OFICIAL
                  </div>
                )}

                <div className="my-2 flex justify-center transform group-hover:scale-108 transition-transform duration-300">
                  <HeroAvatar heroId={hero.id} size={85} talking />
                </div>

                <div className="w-full mt-1">
                  <h3 className="text-lg sm:text-xl font-bold text-[#35206F] font-fredoka mb-1 leading-snug">
                    {hero.name}
                  </h3>
                  <p className="text-xs sm:text-sm text-[#554A6D] font-bold font-nunito mb-3">
                    {hero.movie}
                  </p>
                  <div className="w-full bg-[#FFF9EC] group-hover:bg-[#7AC943] group-hover:text-white text-[#35206F] text-xs sm:text-sm font-extrabold py-2 px-3 rounded-xl border border-[#FFC928]/50 transition-colors font-nunito shadow-2xs">
